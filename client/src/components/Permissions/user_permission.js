...
class User_permission extends Component {
  ...
  receivedData = async () => {
    await axios
      .get('/api/permissions/getPermission/userpermissions')
      .then(res => {
  ...
  handlePermissionChange(username, column_name, e) {
    ...
    usersData.filter(user => user.username == username)[0][column_name] =
      e.target.checked;
    if (!prevupdatedUsersNames.includes(username))
      prevupdatedUsersNames.push(username);
    this.setState({
      usersPermissionsData: usersData,
      updatedUsersNames: prevupdatedUsersNames,
    });
  }
  ...
  handleSave(e) {
    ...
    const usersPermissionsDataToSave = this.state.usersPermissionsData.filter(
      pd => updatedUsersNameskeys.includes(pd.username) == true,
    );
  ...
  render() {
    const page = this.state.usersPermissionsData.slice(
      this.state.offset,
    ...
    return (
      <div className="align-items-center">
          ...
          >
            <div className="modal-dialog modal-dialog-centered" role="document">
              <div className="modal-content">
                ...
                <div className="modal-body">האם לבצע שינויים?</div>
            ...
            <div id={pd.username} key={pd.username} className="row">
              <div className="col border text-center">
                <input
                  ...
                  key=""
                  onClick={e =>
                    this.handlePermissionChange(pd.username, 'image', e)
                  }
                />
              </div>
              <div className="col border text-center">
                <input
                  ...
                  key=""
                  onClick={e =>
                    this.handlePermissionChange(pd.username, 'pdf', e)
                  }
                />
              </div>
              <div className="col border text-center">
                <input
                  ...
                  key=""
                  onClick={e =>
                    this.handlePermissionChange(pd.username, 'print', e)
                  }
                />
              </div>
              <div className="col border text-center">
        ...
        <ReactPaginate
          ...
          pageCount={
            this.state.pageCount != undefined ? this.state.pageCount : 0
          }
