  ...
  try {
    ...
    for (graph of graphList) {
      ...
      graphToAdd.options = {
        ...
        tooltip: { 
          headerFormat: (graph.type=='pie')? `<span dir = "rtl" style="font-size:10px">{point.key}<table></span>`:'' ,
          pointFormat: (graph.type=='pie') ? ('<tr><td style="color:{series.color};padding:0">{series.name}: </td>' +
            '<td style="padding:0"><b>{point.y:.1f} %</b></td></tr>') : '<tr><td style="color:{series.color};padding:0">{series.name}: </td>' +
      ...
      console.log("ROW 144 type:" +graphToAdd.options.chart.type);
      ...
      console.log("PRINTING CATAGORIES");
      console.log(graphToAdd.options.xAxis.catagories);
      ...
      for (series of seriesList) {
        ...
        if (
          ...
          console.log("+++++++logging all valuees: from table sum:"+graphToAdd.options.sum);
          
          for (obj of result.rows) {
            console.log(parseFloat(Object.values(obj)[0] / graphToAdd.options.sum));
      ...
      console.log("Graph is: "+ graphToAdd.options.chart.type);
      if(graphToAdd.options.chart.type=='pie') {
...
exports.addNewDashboard = async (req, res) => {
  ...
  try {
    ...
    result = await dashboarddbpool.query(
      `SELECT COALESCE(MAX(index),0)+1 from public."graphsInfoTable";`,
    );
    ...
    for (graph of dashboardObject.graphList) {
      ...
      result = await dashboarddbpool.query(`INSERT INTO public."graphsInfoTable"(
                ...
                VALUES ((SELECT COALESCE(MAX(index),0)+1 from public."graphsInfoTable"), '${graph.layout.position}', ${graph.layout.width}, 
