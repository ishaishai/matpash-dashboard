    ...
    try {
        ...
        for (graph of graphList) {
            ...
            for (series of seriesList) {
                ...
                console.log(columnNameResult.rows[0]);

                //FIX EHERE EXTRACT OF COLUMN NAME
                const seriesToAdd = {
                    name: columnNameResult.rows, //maybe cahnge this to name
                    colr: series.color,
                    data: []
                }
...
exports.deleteGraphFromDashboard = async (req, res) => {
    ...
    try {
        ...
        //just create new string without the wanted graph and use the following update to erase existence 
        await dashboarddbpool.query(`update public."dashboardPriviledgesTable" set dashboard${id} = '${graphs}';`)//complete query.
